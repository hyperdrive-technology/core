...
export function ScadaEditor() {
  const [tags, setTags] = useState<PLCValue[]>([]);
  const getTags = trpc.plc.getTags.useQuery();
  const subscribeToTags = trpc.plc.subscribeToTags.useMutation();

  useEffect(() => {
    if (getTags.data) {
      setTags(getTags.data);
      void subscribeToTags.mutateAsync({
        tags: getTags.data.map((tag: PLCValue) => tag.name),
      });
    }
  }, [getTags.data, subscribeToTags]);
